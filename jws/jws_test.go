...
func Test_VerifyCompactSerialization(t *testing.T) {
	...
	err := json.Unmarshal([]byte(oobCrypto), &pk)
	if err != nil {
		t.Fatal(fmt.Errorf("error unmarshaling oob crypto payload: %s", err))
	}

	pubKey, err := jwk.PublicFromJwk(pk)
	if err != nil {
